    ...
    internal class Cdetalle
    {
        ...
        public void MostrarFactura(DataGridView tablaFactura)
        {
            ...
            try
            {
                ...
                // Consulta SQL ajustada para manejar clientes que podrían estar eliminados
                string query = "SELECT fac_cod as 'Código Factura', fac_fec as 'Fecha', " +
                               "CONCAT(COALESCE(cli_pnom, ''), ' ', COALESCE(cli_snom, ''), ' ', " +
                               "COALESCE(cli_pape, ''), ' ', COALESCE(cli_sape, '')) as 'Cliente', " +
                               "fac_total as 'Total' " +
                               "FROM factura " +
                               "LEFT JOIN cliente ON factura.cli_id = cliente.cli_cod " +
